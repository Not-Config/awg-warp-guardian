...
1. создаёт ключи и регистрирует свежий WARP-клиент;
...
2. безопасно извлекает только известные списки endpoint-ов, DNS,
   маршрутов и AWG-шаблонов — код страницы не исполняется;
3. локально создаёт новую пару ключей и регистрирует её напрямую в Cloudflare
   WARP API;
4. если прямая регистрация недоступна, пробует API-зеркала из `script.js` с
   теми же HTTP-заголовками, которые отправляет страница;
5. собирает цельный кандидат с сохранёнными параметрами;
6. проверяет его через `awg-quick strip`;
7. запускает профиль и ждёт handshake;
8. параллельно проверяет выбранные сайты и Cloudflare trace;
9. при неудаче откатывает прежний профиль и запрашивает следующий;
10. при успехе оставляет новый профиль активным.
...
- выполняется ли прямая регистрация или используется API-зеркало;
...
`--generator-data` можно повторить: адреса будут использоваться как аварийные
зеркала, если прямая регистрация Cloudflare недоступна. Если параметр не указан,
список зеркал каждый раз читается из `script.js` выбранного сайта.
...
По умолчанию Guardian создаёт `PrivateKey` локально: ключ не отправляется ни на
warp-gen, ни в Cloudflare — при регистрации передаётся только соответствующий
`PublicKey`. Если прямая регистрация недоступна и используется API-зеркало,
зеркало возвращает уже созданный приватный ключ и технически может его знать.
Используйте только доверенный источник и помните, что WARP — не замена
VPN-серверу, которым управляете только вы.
